...
# Begin per-test loop
  ...
  case ${testid} in
    test1 )  test_string="mktae $INDIR/wmap.fits $INDIR/ahist.fits[asphist1] $outfile"
           ;;
  ...
  eval $test_string | tee -a $LOGFILE  2>&1
  ...
  dmdiff $outfile $savfile tol=$SAVDIR/tolerance \
	> /dev/null 2>>$LOGFILE
  if  test $? -ne 0 ; then
